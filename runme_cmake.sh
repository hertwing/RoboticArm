...
usage() {
  echo "Use:"
  echo "  --pi4               cross-build for RPi4 (aarch64)"
  echo "  --pi5               cross-build for RPi5 (aarch64)"
  echo "  -h|--help"
}
...
while [[ $# -gt 0 ]]; do
  case $1 in
    --pi4)
      ...
      RPI_TOOLCHAIN_CMAKE="-DCMAKE_TOOLCHAIN_FILE=${PROJECT_PATH}/rpi4_aarch64_toolchain.cmake"
      BUILD_PATH="${PROJECT_PATH}/build/pi4_arm64"
      EXTRA_ARGS="${EXTRA_ARGS} -DBUILD_PI4_ARM:BOOL=ON"
    ...
    --pi5)
      ...
      BUILD_PATH="${PROJECT_PATH}/build/pi5_arm64"
      EXTRA_ARGS="${EXTRA_ARGS} -DBUILD_PI5_ARM:BOOL=ON"
