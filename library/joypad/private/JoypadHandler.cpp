...
JoypadHandler::JoypadHandler()
{
    ...
    m_shmem_handler = std::make_unique<odin::shmem_wrapper::ShmemHandler<JoypadData>>(
        odin::shmem_wrapper::DataTypes::JOYPAD_SHMEM_NAME, sizeof(JoypadData), true);
    ...
    while (m_joypad_connected && m_run_process)
    {
        while ((m_received_bytes = read(m_joypad_fd, m_buffer, BUFF_SIZE)) > 0)
        {
            ...
            for (int i = 0; i < CONTROL_BYTES; ++i)
            {
                ...
                {
                    parseData(m_buffer);
                    auto data = m_joypad_data.createJoypadData();

                    m_shmem_handler->shmemWrite(&data);
